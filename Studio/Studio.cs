using TinertiaStudio.Entities;
using TinertiaStudio.Controls;
using System;
...
namespace TinertiaStudio {
    public partial class Studio : Form {
        ...
        private TinertiaMemory memory = new TinertiaMemory();
        ...
        public void EnableStudio(bool hooked) {
            if (hooked) {
                string fileName = Path.Combine(Path.GetDirectoryName(memory.Program.MainModule.FileName), "Tinertia.tas");
        ...
        private void UpdateStatusBar() {
            if (memory.IsHooked) {
                string posVel = memory.TASPosition().ToString() + memory.TASVelocity().ToString();
                lblStatus.Text = "F(" + (currentFrame > 0 ? currentFrame + "/" : "") + totalFrames + ") (" + memory.Level().ToString() + "-" + memory.SectionFinished().ToString() + ") Actions(" + memory.PendingActions().ToString() + ")\r\n" + posVel;
            } else {
                ...
                if (old != null) {
                    ...
                    if (text != line) {
                        ...
                        if (!string.IsNullOrEmpty(line)) {
                            ...
                            if (index < 0) { index = 0; }
                            if (index > 4 && old.Shoot == input.Shoot) { index = 4; }
                            if (old.Frames == input.Frames && old.ZeroPadding == input.ZeroPadding && old.Shoot == input.Shoot) { index = 4; }
